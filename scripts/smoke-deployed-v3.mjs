...
try {
  ...
  const readableCard = page.locator('#shelf-magazine-rack .cover-card, #shelf-comics .cover-card').first();
  if (await readableCard.count()) {
    await readableCard.click();
    const readInApp = page.getByRole('button', { name: 'Read in App', exact: true });
    if (await readInApp.count()) {
      await readInApp.click();
      await page.waitForTimeout(750);
      if (!(await page.locator('#reader.reader-overlay.open').count())) failures.push('reader did not open');
    ...
    } else failures.push('Readable shelf detail had no in-app reader action');
  } else failures.push('No readable shelf card was available to exercise the reader');
