...
try {
  ...
  const comic = page.locator('#shelf-comicbookplus .cover-card').first();
  if (await comic.count()) {
    await comic.click();
    ...
    } else failures.push('Comic Book Plus detail modal had no in-app reader action');
  } else failures.push('Comic Book Plus had no card to exercise the reader');
