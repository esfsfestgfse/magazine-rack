...
function publicLibraryItem(row) {
  ...
  return { ...publicItem({ id: row.item_id, ...row, sourceName: row.source, coverUrl: row.cover_url, sourceUrl: row.source_url, readerUrl: row.reader_url, pageCount: row.page_count, lastSeenAt: row.last_seen_at, readable: Boolean(row.readable), readerKind: row.reader_kind || 'none', coverQuality: Number(row.cover_quality) || 0, coverScore: Number(row.cover_score) || 0, rights: row.rights || '', metadata }), note: row.note || '', savedAt: row.saved_at };
}
...
export async function handleLibrary(request, env, url, requestId) {
  ...
  if (request.method === 'GET') {
    ...
    const rows = await env.DB.prepare('SELECT l.item_id, l.note, l.saved_at, c.source, c.title, c.series_title, c.issue_number, c.volume_number, c.creator, c.year, c.genre, c.description, c.cover_url, c.source_url, c.reader_url, c.page_count, c.metadata_json, c.last_seen_at, c.access, c.readable, c.reader_kind, c.cover_quality, c.cover_score, c.availability_json, c.rights, c.access_checked_at FROM library_entries l LEFT JOIN catalog_items c ON c.id = l.item_id WHERE l.library_key = ? ORDER BY l.saved_at DESC LIMIT 500').bind(maybeKey).all();
