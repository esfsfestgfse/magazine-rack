...
function dbItem(row) {
  ...
  return { ...row, source, sourceName: SOURCE_NAMES[source] || row.source, coverUrl: row.cover_url, sourceUrl: row.source_url, readerUrl: row.reader_url, pageCount: row.page_count, lastSeenAt: row.last_seen_at, metadata };
}

async function persist(env, items) {
  if (!env.DB || !items.length) return;
  const timestamp = new Date().toISOString();
  const statements = items.slice(0, 90).map((item) => env.DB.prepare(`INSERT INTO catalog_items (id, source, source_id, title, creator, year, genre, description, cover_url, source_url, reader_url, page_count, metadata_json, first_seen_at, last_seen_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET source=excluded.source, title=excluded.title, creator=excluded.creator, year=excluded.year, genre=excluded.genre, description=excluded.description, cover_url=excluded.cover_url, source_url=excluded.source_url, reader_url=excluded.reader_url, page_count=excluded.page_count, metadata_json=excluded.metadata_json, last_seen_at=excluded.last_seen_at`).bind(item.id, item.source, item.sourceId, item.title, item.creator, item.year, item.genre, item.description, item.coverUrl, item.sourceUrl, item.readerUrl, item.pageCount, JSON.stringify(item.metadata || {}), timestamp, timestamp));
...
async function stored(env, query, genre, page, source) {
  ...
  const count = await env.DB.prepare(`SELECT COUNT(*) AS total FROM catalog_items WHERE (? = '' OR title LIKE ? OR creator LIKE ? OR description LIKE ?) AND (? = '' OR lower(genre) = lower(?))${sourceClause}`).bind(...countValues).first();
...
export async function handleCatalogSearch(request, env, ctx, requestId) {
  ...
  if (!items.length && env.DB) { const fallback = await stored(env, query, genre, page, source); items = fallback.items; total = fallback.total; }
