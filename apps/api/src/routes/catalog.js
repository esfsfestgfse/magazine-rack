...
function dbItem(row) {
  ...
  return { ...row, source, sourceName: SOURCE_NAMES[source] || row.source, coverUrl: row.cover_url, sourceUrl: row.source_url, readerUrl: row.reader_url, pageCount: row.page_count, issueMonthDay: row.issue_month_day || '', lastSeenAt: row.last_seen_at, readable: Boolean(row.readable), readerKind: row.reader_kind || 'none', coverQuality: Number(row.cover_quality) || 0, coverScore: Number(row.cover_score) || 0, rights: row.rights || '', availability, metadata };
}
...
async function persist(env, items) {
  ...
  const catalogStatements = items.slice(0, 90).map((item) => env.DB.prepare(`INSERT INTO catalog_items (id, source, source_id, title, series_title, issue_number, volume_number, creator, year, issue_month_day, genre, description, cover_url, source_url, reader_url, page_count, metadata_json, first_seen_at, last_seen_at, access, readable, reader_kind, cover_quality, cover_score, availability_json, rights, access_checked_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET source=excluded.source, title=excluded.title, series_title=excluded.series_title, issue_number=excluded.issue_number, volume_number=excluded.volume_number, creator=excluded.creator, year=excluded.year, issue_month_day=excluded.issue_month_day, genre=excluded.genre, description=excluded.description, cover_url=excluded.cover_url, source_url=excluded.source_url, reader_url=excluded.reader_url, page_count=excluded.page_count, metadata_json=excluded.metadata_json, last_seen_at=excluded.last_seen_at, access=excluded.access, readable=excluded.readable, reader_kind=excluded.reader_kind, cover_quality=excluded.cover_quality, cover_score=excluded.cover_score, availability_json=excluded.availability_json, rights=excluded.rights, access_checked_at=excluded.access_checked_at`).bind(item.id, item.source, item.sourceId, item.title, item.series || '', item.issue || '', item.volume || '', item.creator, item.year, monthDay(item.metadata?.date || item.year || item.title), item.genre, item.description, item.coverUrl, item.sourceUrl, item.readerUrl, item.pageCount, JSON.stringify(item.metadata || {}), timestamp, timestamp, item.access || 'catalog', item.readable === true ? 1 : 0, item.readerKind || 'none', Number(item.coverQuality) || 0, Number(item.coverScore) || 0, JSON.stringify(item.availability || {}), item.rights || '', item.accessCheckedAt || timestamp));
...
async function stored(env, query, genre, page, source, newspaperMonthDay) {
  ...
  const select = `SELECT id, source, source_id, title, series_title, issue_number, volume_number, creator, year, issue_month_day, genre, description, cover_url, source_url, reader_url, page_count, metadata_json, last_seen_at, access, readable, reader_kind, cover_quality, cover_score, availability_json, rights, access_checked_at FROM catalog_items`;
  const result = await env.DB.prepare(`${select}${where} ORDER BY cover_score DESC, cover_quality DESC, last_seen_at DESC LIMIT 30 OFFSET ?`).bind(...values).all();
