...
function shelfDetailView(shelfId) {
  const shelf = SHELVES.find((entry) => entry.id === shelfId);
  if (!shelf) return `<div class="view wall-empty"><strong>Shelf not found.</strong><a href="#/racks">Back to the rack ${icon('arrow')}</a></div>`;
  const current = rackState(shelf);
  const grouped = new Map();
  current.docs.forEach((doc) => {
    const key = titleOf(doc).replace(/(?:\s+v(?:ol)?\.?\s*\d+|\s+#?\d+|\s+\d{4}[-/]\d{2}.*)$/i, '').trim() || titleOf(doc);
    if (!grouped.has(key)) grouped.set(key, []);
    grouped.get(key).push(doc);
  });
  return `<div class="view shelf-detail-view"><div class="page-heading"><div><span class="eyebrow">${shelf.secondary ? 'SECONDARY SOURCE' : 'LIVE SHELF'}</span><h1>${escapeHtml(shelf.title)}<br><i>issue index.</i></h1><p>${escapeHtml(shelf.description || rackDescription(shelf))}</p></div><div class="page-heading-actions"><a class="secondary-button" href="#/racks">${icon('arrow')} Back to racks</a><button class="primary-button" data-action="refresh-rack" data-id="${escapeHtml(shelf.id)}">${icon('refresh')} Refresh shelf</button></div></div><div class="detail-toolbar"><span>${current.total ? `${current.total.toLocaleString()} available` : 'Loading snapshot'}</span><span>${grouped.size} series groups</span><span>${current.fallback ? 'Cached fallback' : current.loaded ? 'Live snapshot' : 'Checking source'}</span></div><div class="series-groups">${current.loading && !current.docs.length ? loadingCards(12) : [...grouped.entries()].map(([series, issues]) => `<section class="series-group"><div class="series-heading"><div><span class="eyebrow">SERIES / RUN</span><h2>${escapeHtml(series)}</h2></div><span>${issues.length} issue${issues.length === 1 ? '' : 's'}</span></div><div class="series-grid">${issues.map((doc, index) => coverMarkup(doc, index)).join('')}</div></section>`).join('') || '<div class="wall-empty"><strong>No issues in this snapshot.</strong><span>Refresh the shelf or try again later.</span></div>'}</div>${current.hasMore && current.loaded ? `<button class="load-more-detail" data-action="load-more" data-id="${escapeHtml(shelf.id)}">Load more issues ${icon('arrow')}</button>` : ''}</div>`;
}
...
function render() {
  const viewMarkup = state.view === 'library' ? libraryView() : state.view === 'wall' ? wallView() : state.view === 'about' ? aboutView() : state.view === 'search' ? searchView() : state.view === 'shelf' ? shelfDetailView(state.shelfId) : racksView();
...
function routeFromHash() { const raw = window.location.hash.replace(/^#\/?/, '') || 'racks'; const path = raw.split('?')[0].split('/'); const view = path[0]; const query = raw.includes('?') ? raw.slice(raw.indexOf('?') + 1) : ''; const params = new URLSearchParams(query); return { view: ['racks', 'wall', 'library', 'about', 'search', 'shelf'].includes(view) ? view : 'racks', shelfId: view === 'shelf' ? decodeURIComponent(path[1] || '') : '', query: params.get('q') || '' }; }
async function loadRoute(route) { if (route.view === 'search' && route.query) await loadSearch(); if (route.view === 'shelf' && route.shelfId) { const shelf = SHELVES.find((entry) => entry.id === route.shelfId); if (shelf && !rackState(shelf).loaded) queueShelf(shelf.id, false); } }
window.addEventListener('hashchange', async () => { const route = routeFromHash(); state.view = route.view; state.shelfId = route.shelfId; state.query = route.query; state.search = route.view === 'search' && route.query ? { ...state.search, loading: true } : { docs: [], total: 0, loading: false, error: '' }; render(); await loadRoute(route); });
